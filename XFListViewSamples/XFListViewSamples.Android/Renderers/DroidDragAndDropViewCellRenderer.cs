...
[assembly: ExportRenderer(typeof(DraggableViewCellControl), typeof(DroidDragAndDropViewCellRenderer))]
namespace XFListViewSamples.Droid.Renderer
{
    public class DroidDragAndDropViewCellRenderer : ViewCellRenderer
    {
        ...
        private void CellcoreOnDrag(object sender, View.DragEventArgs args)
        {
            ...
            switch (args.Event.Action)
            {
                ...
                case DragAction.Drop:
                    ...
                    if (FirstIndex != -1)
                    {
                        var firstItem = Items[FirstIndex - 1];
                        if (firstItem != null)
                        {
                            Items.RemoveAt(FirstIndex - 1);
                            Items.Insert(SecondIndex - 1, firstItem);
